...
const limiter = rateLimit({
  windowMs: 15 * 60 * 1000,
  max: 100,
  message: 'Too many requests from this IP, ay bhai ja na q mastia kr rha ha.'
})
...
restoreOverwrittenFilesWithOriginals().then(() => {
  ...
  /* Global rate limiting to protect login attempts and brute force abuse */
  app.use(limiter)

  /* Restrictive CORS policy for frontend access only */
  ...
  /* Reject session identifiers in URLs */
  app.use((req: Request, res: Response, next: NextFunction) => {
    const sessionUrlKeys = ['jsessionid', 'phpsessid', 'sessionid', 'sid']
    if (Object.keys(req.query).some(key => sessionUrlKeys.includes(key.toLowerCase()))) {
      return res.status(400).json({ error: 'Session identifiers must not be passed in the URL' })
    }
    next()
  })
  ...
  /* Secure API route protected by API key */
  app.get('/secure-data', authenticateAPI, (req: Request, res: Response) => {
...
export async function start (readyCallback?: () => void) {
  ...
  try {
    ...
    if (errorMessage.includes('SQLITE_IOERR') || errorMessage.includes('disk I/O error')) {
      logger.warn('Database I/O error detected. Attempting recovery by removing corrupted database file...')
      const dbPath = 'data/juiceshop.sqlite'
      const journalPath = 'data/juiceshop.sqlite-journal'
      
      // Remove corrupted database file and journal
      if (fs.existsSync(dbPath)) {
        try {
          fs.unlinkSync(dbPath)
          logger.info('Removed corrupted database file')
        } catch (unlinkErr) {
          logger.error(`Failed to remove database file: ${utils.getErrorMessage(unlinkErr)}`)
        }
      }
      if (fs.existsSync(journalPath)) {
        try {
          fs.unlinkSync(journalPath)
          logger.info('Removed corrupted database journal file')
        } catch (unlinkErr) {
          logger.error(`Failed to remove journal file: ${utils.getErrorMessage(unlinkErr)}`)
        }
      }
      
      // Retry sync with fresh database
      logger.info('Retrying database sync with new database file...')
      await sequelize.sync({ force: true })
